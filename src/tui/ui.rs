...
fn draw_help(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let lines = [
        ...
        "",
        "Enter, u    update selected crate: the entry is read and the",
        "            registry asked now, the plan is shown for a y, and the",
        "            build runs in the panel",
        "U           update every unpinned crate: the registry is asked",
        "            about all of them — the list is refreshed as `r` would",
        "            refresh it — the plan is shown for a y, and the builds",
        "            run here one at a time; a failure does not stop the rest",
        "i           install: NAME[@VERSION]... [--locked]  (@VERSION pins);",
        "            builds in the Build panel — several crates are one",
        "            operation over one plan, built in order; a failure is",
        "            the member's and does not stop the rest, as on the",
        "            command line — only a cancel does",
        "x           remove selected crate (asks first; in place, with only",
        "            the sudo prompt borrowing the screen — the terminal is",
        "            handed over only if sudo cannot cache credentials)",
        ...
        "            door a cancel is too late for the current crate — an",
        "            install finishes placing, a migration proceeds through",
        "            its retirement attempt — and a batch still stops after",
        "            it: no later member starts;",
        ...
        "            (in place; the terminal only for a sudo that cannot",
        "            cache credentials)",
        ...
        "Every build runs inside the TUI: the build first, then sudo when",
        "placement begins — the first lock in a prefix is the exception,",
        "asked for before the build. A failed build keeps its last lines",
        "and the log path in the details panel.",
        "Everything runs here. The terminal is borrowed for a sudo prompt,",
        "and handed over entirely in one case only: a sudo that caches",
        "nothing, where the work cannot run noninteractively at all.",
        "",
