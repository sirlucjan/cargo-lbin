...
In the interface those are `t` and `T`. The single rebuild runs in the panel, like a single-crate install or the rebuild behind `D` — cancellable, with its warnings on the record. The sweep hands the terminal over, as `U` does and for the same reason: it prints a plan and asks, and that exchange has a better home in the terminal than behind a panel. A pinned crate is rebuilt by `t` like any other, because `t` restates the pin rather than moving off it; the refusal a pin raises belongs to `install NAME`.
...
| `T` | Rebuild every crate under the prefix; the plan and its confirmation happen in the terminal |
