...
The sweeps work the same way. `U` snapshots the manifest under a shared lock, releases it before the registry requests and the confirmation — a check across fifty crates holds nothing — shows the plan crate by crate, and then holds the exclusive lock across the updates, revalidating each member against the plan and skipping one that moved. Pinned entries are not in the plan, and how many were held back is said rather than implied.
