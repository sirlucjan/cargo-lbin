...
A plain `install` of an already managed crate rebuilds it only when the artifact specification changes: a different effective version, or `--locked` changing in either direction — installing without `--locked` re-declares the policy off, which is also how a stored `--locked` is cleared (to move to a newer version while *keeping* the policy, use `update`). When the artifact specification does not change — same effective version, same policy — the command refuses and names the verbs that state the intent: `install --reinstall` rebuilds the entry exactly as stored, and `pin` sets the hold without a build, because a pin is never a reason to build. The build itself, when it happens, is a full rebuild and replacement in a fresh staging directory: binaries are replaced and the version is resolved afresh.
...
In the interface those are `t` and `T`, and both run in the panel. The single rebuild is a build like any other — cancellable, with its warnings on the record. The sweep reads the prefix under a shared lock, shows the frozen plan and asks about it, then holds the exclusive lock across every rebuild, re-checking each entry against the plan that was confirmed and skipping one that moved in between; a failure does not stop the rest, because the question a sweep answers is how many survived. A pinned crate is rebuilt by `t` like any other, because `t` restates the pin rather than moving off it; the refusal a pin raises belongs to `install NAME`.
...
A pinned crate is refused — `unpin` first — because the downgrade would re-pin it to another version, re-interpreting the standing pin instead of preserving it.
...
| `Enter`, `u` | Update the selected crate: the manifest is read and the registry asked, the plan confirmed, and the build runs in the panel |
| `U` | Update every unpinned crate: a fresh check, the plan shown and confirmed, then the builds here one at a time |
| `i` | Open the install line (`NAME[@VERSION]... [--locked]`; `@VERSION` pins) |
| `x` | Remove the selected crate after TUI confirmation; in place, with only the `sudo` prompt borrowing the screen — the terminal is handed over only when `sudo` cannot cache credentials |
...
| `p` | Pin or unpin the selected crate; in place, with the same one exception as `x` |
| `D` | Downgrade the selected crate: the panel offers the older versions, a digit installs one |
| `t` | Rebuild the selected crate exactly as installed — same version, pin and `--locked`, new artifacts |
| `T` | Rebuild every crate under the prefix: read, confirmed as a whole, then built here one at a time — a failure does not stop the rest |
...
A batch install is one operation: a single worker takes the prefix's lock once, checks the whole plan — pins, duplicates — before the first build, then builds in order, each member its own unit from build to manifest commit. A member that fails or cannot be placed is reported and the batch carries on, exactly as `cargo lbin install a b c` does: an error in one crate is a statement about that crate, and only an explicit cancel speaks for the members not yet started. The summary distinguishes a crate that failed to build from one that built and could not be placed, because sending someone to look for a compiler error that does not exist is its own kind of untruth; any shortfall exits non-zero. Credentials are not reserved up front: each member asks at its own door.
