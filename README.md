...
A batch install is one operation: a single worker takes the prefix's lock once, checks the whole plan — pins, duplicates — before the first build, then builds in order and stops at the first member that does not install, exactly as `cargo lbin install a b c` does. Its summary distinguishes a crate that failed to build from one that built and could not be placed, because sending someone to look for a compiler error that does not exist is its own kind of untruth. Credentials are not reserved up front: each member asks at its own door, and what the batch promises is that after a stop or a refusal it starts nobody else.
