...
In the interface those are `t` and `T`, and both run in the panel. The single rebuild is a build like any other — cancellable, with its warnings on the record. The sweep reads the prefix under a shared lock, shows the frozen plan and asks about it, then holds the exclusive lock across every rebuild. A pinned crate is rebuilt by `t` like any other, because `t` restates the pin rather than moving off it; the refusal a pin raises belongs to `install NAME`.

In the interface that sweep is `T`, and it runs in the panel: the prefix is read under a shared lock, the whole set is confirmed as one question, and then a single worker holds the exclusive lock across every rebuild — re-checking each entry against the plan that was confirmed and skipping one that moved in between. A failure does not stop the rest, because the question a sweep answers is how many survived.
...
A single-crate install builds in place, inside its own transient Build panel, and a removal or a pin flip runs in place whether or not it needs a password — when it does, only the prompt borrows the screen, and the terminal is handed over only for a `sudo` that cannot cache credentials at all; `sudo` is asked for at placement rather than before the build, and the interface steps aside for that prompt; a migration out of a privileged prefix into a writable one escalates only after the build — the privileged half is retiring the source — and says so before the build starts, so a prompt arriving then is expected rather than startling (whether `sudo` asks at all depends on its own timestamp, so the notice promises the escalation, not the prompt); the CLI says the same before each crate's build; a batch install is one operation and runs in the panel too: a single worker takes the prefix's lock once, checks the whole plan — pins, duplicates — before the first build, then builds in order and stops at the first member that does not install, exactly as `cargo lbin install a b c` does — and the summary distinguishes a crate that failed to build from one that built and could not be placed, because sending someone to look for a compiler error that does not exist is its own kind of untruth. Credentials are still asked for at the placement door of each member, not reserved up front: what a batch promises is that after a stop or a refusal it starts nobody else, not that the whole plan could be authorized before the first build; the last build workflow to move, `U`, works the same way: the manifest is snapshotted under a shared lock which is released before the registry requests and the confirmation, so a check across fifty crates holds nothing; the plan is shown crate by crate — pinned entries are not in it, and how many were held back is said rather than implied; and one worker then holds the exclusive lock across the updates, revalidating each member against the plan and skipping one that moved. What is left of the old handoff is one case: a `sudo` that caches nothing, where the work cannot run noninteractively at all — a build discovers it before it starts, a removal or a pin flip at its door where Cargo diagnostics, the update confirmation and password prompts behave exactly as they do outside the TUI, and the interface returns afterwards.
